...
from pybci_lib.Utils.Logger import Logger
...
class LSLScanner:
    ...
    def ScanMarkerStreams(self):
        ...
        if self.markerStreamPredefined:
            # Look for the specific marker stream
            for s in markerStreams:
                if s.info().name() in self.markerStreamName:
                    self.markerStream = s
                    break
            
            if self.markerStream is None:
                self.logger.log(Logger.WARNING, " Predefined LSL Marker Stream name not found.")
                self.logger.log(Logger.WARNING, " Available Streams: " + str([s.info().name() for s in markerStreams]))
            elif len(markerStreams) > 1:
                self.logger.log(Logger.WARNING, " Multiple Marker streams available, but the desired stream '" + str(self.markerStreamName) + "' was found.")
        else:
